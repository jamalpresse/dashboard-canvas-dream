...
export default function Search() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 to-gray-800 p-6 animate-fade-in" dir={dir}>
      <Card className="w-full max-w-4xl mx-auto shadow-md hover:shadow-lg transition-all duration-300">
        <CardHeader>
          <CardTitle className="text-2xl font-semibold text-gray-800">{t('search', 'title')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <textarea
            ...
            className={`w-full h-24 border border-gray-300 rounded-lg p-3 resize-none focus:outline-none focus:ring-2 focus:ring-snrt-red ${isQueryRTL ? 'text-right' : 'text-left'}`}
          />
          <div className={`flex space-x-3 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
            <button 
              onClick={handlePaste} 
              className="px-4 py-2 bg-snrt-red text-white rounded-md hover:bg-red-700 shadow-sm transition-colors"
            >
              {t('search', 'paste')}
            </button>
            <button 
              onClick={handleSearch} 
              className="px-4 py-2 bg-black text-white rounded-md hover:bg-gray-800 shadow-sm transition-colors" 
              disabled={loading}
            >
            ...
            {!error && (
              <>
                ...
                <div className={`flex justify-end space-x-2 mt-2 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  <button 
                    onClick={handleClear} 
                    className="px-3 py-1 bg-gray-200 text-black rounded-md hover:bg-gray-300 shadow-sm transition-colors"
                  >
                    {t('search', 'clear')}
                  </button>
                  <button 
                    onClick={handleCopy} 
                    className="px-3 py-1 bg-snrt-red text-white rounded-md hover:bg-red-700 shadow-sm transition-colors"
                  >
                    {t('search', 'copy')}
                  </button>
                </div>
          ...
          <div className={`flex flex-col sm:flex-row gap-4 w-full mt-8 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <Link 
              to="/improve" 
              className="flex-1 px-6 py-4 bg-snrt-red text-white rounded-xl shadow hover:shadow-lg transition-all hover:-translate-y-1 flex items-center justify-center gap-3"
            >
              <Wrench size={20} />
            ...
            <Link 
              to="/translation" 
              className="flex-1 px-6 py-4 bg-black text-white rounded-xl shadow hover:shadow-lg transition-all hover:-translate-y-1 flex items-center justify-center gap-3 hover:bg-snrt-red/90"
            >
              <Globe size={20} />
