  ...
  return <div className="min-h-screen bg-gradient-to-br from-gray-950 to-gray-800 p-6 animate-fade-in" dir={dir}>
      <Card className="w-full max-w-4xl mx-auto shadow-md hover:shadow-lg transition-all duration-300">
        ...
        <CardContent className="space-y-6">
          <textarea value={query} onChange={e => setQuery(e.target.value)} placeholder={t('search', 'placeholder')} dir={isQueryRTL ? 'rtl' : 'ltr'} className={`w-full h-24 border border-gray-300 rounded-lg p-3 resize-none focus:outline-none focus:ring-2 focus:ring-snrt-red ${isQueryRTL ? 'text-right' : 'text-left'}`} />
          ...
          <div>
            ...
            {!error && <>
                <textarea readOnly value={loading ? '' : result || t('search', 'noResult')} dir={isResultRTL ? 'rtl' : 'ltr'} className={`w-full h-48 border border-gray-300 rounded-lg p-3 resize-none bg-white/80 backdrop-blur-sm shadow-inner focus:outline-none ${isResultRTL ? 'text-right' : 'text-left'}`} />
