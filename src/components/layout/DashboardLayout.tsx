  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="snrt-header flex flex-col">
        ...
        <nav className="snrt-nav overflow-x-auto scrollbar-hide bg-red-600 text-white">
          <div className="flex space-x-1 px-4 py-2">
            {categories.map((category, index) => (
              <Link 
                key={index} 
                to={category.href}
                className="whitespace-nowrap px-3 py-1.5 text-sm font-medium hover:bg-red-700 hover:text-white rounded-sm transition-colors"
              >
                {category.name}
              </Link>
            ))}
