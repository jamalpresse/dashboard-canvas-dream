...
export default function DashboardLayout() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50">
      {/* Mobile menu button */}
      {isMobile && <button className="fixed top-4 right-4 z-50 bg-white rounded-full p-2 shadow-md text-purple-700" onClick={toggleSidebar}>
          {sidebarOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
        </button>}
      
      {/* Sidebar */}
      <aside className={cn("fixed inset-y-0 left-0 z-40 flex h-full w-64 flex-col bg-white/90 backdrop-blur-md border-r border-purple-100 shadow-lg transition-transform duration-300 ease-in-out", isMobile && !sidebarOpen ? "-translate-x-full" : "translate-x-0")}>
        ...
        <nav className="flex-1 overflow-auto p-4">
          <div className="space-y-1 pb-4">
            ...
            isActive
          }) => cn("flex items-center gap-4 rounded-lg px-4 py-3 text-lg font-medium transition-all duration-200 ease-in-out", isActive ? "bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-md" : "text-gray-700 hover:bg-purple-50 hover:text-purple-700 hover:shadow-sm")}>
                <span className="flex-shrink-0">{item.icon}</span>
                <span>{item.title}</span>
              </NavLink>)}
            
            {/* SNRT News button */}
            <button onClick={toggleNewsFrame} className="flex items-center gap-4 rounded-lg px-4 py-3 transition-all duration-200 ease-in-out w-full text-left hover:bg-purple-50 hover:shadow-sm text-pink-600 text-xl font-bold">
      ...
      {showNewsFrame && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
