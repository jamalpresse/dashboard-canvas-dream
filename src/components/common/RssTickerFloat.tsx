...
export function RssTickerFloat() {
  ...
  useEffect(() => {
    async function fetchRSS() {
      try {
        ...
        const rssUrl = lang === "fr" ? "https://snrtnews.com/fr/rss_fr.xml" : "https://snrtnews.com/rss.xml";
  ...
  return <div className="fixed bottom-0 left-0 right-0 z-50 backdrop-blur-md border-t border-purple-200 shadow-lg bg-purple-800">
      <div className="container mx-auto overflow-hidden">
        ...
        <div className={`flex items-center ${isRTL ? 'flex-row-reverse px-6' : 'flex-row px-6'} py-3`}>
          ...
          <div className="flex-1 overflow-hidden">
            <div className={cn("marquee whitespace-nowrap",
          // Use RTL animation for French and LTR for Arabic
          lang === "fr" ? "marquee-rtl" : "marquee-ltr")}>
              {rssItems.length > 0 ? rssItems.map((item, index) => <a key={index} href={item.link} target="_blank" rel="noopener noreferrer" className={`inline-block ${isRTL ? 'ml-12' : 'mr-12'} text-snrt-red hover:underline hover:text-red-700`}>
                    {item.title}
